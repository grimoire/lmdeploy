...
def _add_seq(session, length: int, fill_value: int = 0):
    ids = [fill_value] * length
    return session.add_sequence(ids)
...
class TestScheduler:
    ...
    def test_schedule_base(self, scheduler, block_size, num_gpu_blocks):
        ...
        seq = _add_seq(session, block_size * num_blocks)
    ...
    def test_update(self, scheduler, block_size, num_gpu_blocks):
        ...
        seq1 = _add_seq(session1, block_size * 1)
        scheduler.add_sequence(seq1)

        session_id2 = 1
        session2 = scheduler.add_session(session_id2)
        seq2 = _add_seq(session2, block_size * 2)
        scheduler.add_sequence(seq2)
        seq3 = _add_seq(session2, block_size * 3)
    ...
    def test_eviction(self, scheduler, block_size, num_gpu_blocks):
        block_manager = scheduler.block_manager
        session_id = 0
        session = scheduler.add_session(session_id)

        # test: add 3 seq
        seq1 = _add_seq(session, block_size * 1, fill_value=1)
        scheduler.add_sequence(seq1)
        seq2 = _add_seq(session, block_size * 2, fill_value=2)
        scheduler.add_sequence(seq2)
        ...
        assert len(output.swap_out_map) == 0
