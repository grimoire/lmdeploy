...
class TestChunkGatedDeltaRuleE2E:
    ...
    @pytest.mark.parametrize(
        'batch,seqlen,heads,v_heads,head_dim,value_dim,dtype,use_qk_l2norm_in_kernel,output_final_state',
        [
            (1, 128, 4, 4, 128, 128, torch.bfloat16, False, False),
            (1, 64, 4, 8, 128, 128, torch.bfloat16, True, True),
        ],
    )
    def test_fixed(self, batch, seqlen, heads, v_heads, head_dim, value_dim, dtype, use_qk_l2norm_in_kernel,
                   output_final_state):
        ...
        g = -2.0 * torch.rand(batch, seqlen, v_heads, dtype=torch.float32)
        beta = torch.rand(batch, seqlen, v_heads, dtype=dtype)
        initial_state = torch.randn(batch, v_heads, head_dim, value_dim, device='cuda', dtype=torch.float32) * 0.05
    ...
    @pytest.mark.parametrize(
        'cu_seqlens,heads,v_heads,head_dim,value_dim,dtype,output_final_state',
        [
            ([0, 65, 129], 4, 4, 128, 128, torch.bfloat16, False),
            ([0, 127, 257], 16, 32, 128, 128, torch.bfloat16, True),
        ],
    )
